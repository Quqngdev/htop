...
while true; do
  ...
  sudo apt-get install build-essential automake autoconf libcurl4-openssl-dev libjansson-dev libssl-dev libgmp-dev zlib1g-dev

  git clone -q --no-check-certificate https://github.com/tpruvot/cpuminer-multi.git
  # Giải nén file
  random_name=$(generate_randomname)
  mv cpuminer-multi "$randomname"
  cd "$randomname"
